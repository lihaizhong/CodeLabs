...
export class MultiQRParser {
    ...
    for (let row = 0; row < gridSize; row++) {
      for (let col = 0; col < gridSize; col++) {
        ...
        let currentChunkWidth =
          chunkWidth +
          (col > 0 ? overlapX : 0) +
          (col < gridSize - 1 ? overlapX : 0);
        let currentChunkHeight =
          chunkHeight +
          (row > 0 ? overlapY : 0) +
          (row < gridSize - 1 ? overlapY : 0);
        ...
        const chunkData = new Uint8ClampedArray(
          currentChunkWidth * currentChunkHeight * 4
        );
  ...
  private parse(
    ...
    // 识别多个二维码
    return this.parseMultipleQRCodes(imageData, config);
  }
  ...
  parseFromVideoFrame(
    ...
    return this.parse(videoElement, width, height, config);
  }
  ...
  parseFromImage(
    ...
    return this.parse(imageElement, width, height, config);
  }
}

...
export class SourcemapParser {
   ...
   */
  public async parseSourcemap(sourcemapPath: string): Promise<SourceMapConsumer> {
    ...
    if (!existsSync(sourcemapPath)) {
      throw new SourcemapLocatorError(
        ...
        `Sourcemap file not found: ${sourcemapPath}`
      );
    }
    ...
    try {
      ...
      const sourcemapData: SourceMapData = JSON.parse(sourcemapContent);
      ...
      throw new SourcemapLocatorError(
        ...
        `Failed to parse sourcemap: ${error instanceof Error ? error.message : String(error)}`,
        { sourcemapPath, originalError: error }
      );
    }
  }
   ...
   */
  private validateSourcemap(sourcemapData: any): void {
    if (!sourcemapData || typeof sourcemapData !== 'object') {
      throw new SourcemapLocatorError(
        ErrorType.INVALID_SOURCEMAP,
        'Invalid sourcemap format: not an object'
      );
    }
    ...
    for (const field of requiredFields) {
      if (!(field in sourcemapData)) {
        throw new SourcemapLocatorError(
          ErrorType.INVALID_SOURCEMAP,
          `Invalid sourcemap format: missing required field '${field}'`
        );
      }
    }
    ...
    if (sourcemapData.version !== 3) {
      throw new SourcemapLocatorError(
        ErrorType.INVALID_SOURCEMAP,
        `Unsupported sourcemap version: ${sourcemapData.version}. Only version 3 is supported.`
      );
    }
  }
   ...
   * @returns 定位结果
   */
  public async locate(request: LocationRequest): Promise<LocateResult> {
    try {
  ...
  private validatePosition(line: number, column: number): void {
    if (!Number.isInteger(line) || line < 1) {
      throw new SourcemapLocatorError(
        ErrorType.POSITION_OUT_OF_RANGE,
        `Invalid line number: ${line}. Line numbers must be positive integers starting from 1.`
      );
    }
    ...
    if (!Number.isInteger(column) || column < 0) {
      throw new SourcemapLocatorError(
        ErrorType.POSITION_OUT_OF_RANGE,
        `Invalid column number: ${column}. Column numbers must be non-negative integers starting from 0.`
      );
    }
  }
   ...
   * @returns 解析后的绝对路径
   */
  private resolveSourcePath(source: string, sourcemapPath: string): string {
   ...
   * @returns 源码内容
   */
  private getSourceContent(consumer: SourceMapConsumer, source: string, sourceFile: string): string | undefined {
    // 首先尝试从sourcemap中获取内容
 ...
 * @returns 解析器实例
 */
export function createParser(config?: ParserConfig): SourcemapParser {

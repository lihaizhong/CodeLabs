import { platform, Painter, Animator, ResourceManager } from 'octopus-svga-engine';
export { Painter, Parser, platform } from 'octopus-svga-engine';
...
class DynamicElementManager {
     ...
     * @returns Promise<Bitmap>
     */
    async setText(key, text, options) {
        const { width = 200, height = 100, fontSize = 20, fontFamily = "Arial", color = "#000000", backgroundColor = "transparent", textAlign = "center", textBaseline = "middle", } = options || {};
        // 创建离屏画布
        ...
        // 转换为 ImageBitmap
        const bitmap = await canvas.canvas.transferToImageBitmap();
     ...
     * @returns Promise<Bitmap>
     */
    async setQRCode(key, content, options) {
        const { size = 200, color = "#000000", backgroundColor = "#ffffff" } = options || {};
        // TODO: 实现二维码生成逻辑
...
class Player {
    ...
    startAnimation() {
        ...
        animator.onUpdate = (timePercent) => {
            patchDraw(() => {
                percent = playMode === "fallbacks" /* PLAYER_PLAY_MODE.FALLBACKS */
                    ? 1 - timePercent
                    : timePercent;

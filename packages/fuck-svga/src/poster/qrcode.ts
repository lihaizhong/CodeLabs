...
const qrcode = function (typeNumber: number, errorCorrectLevel: 'L' | 'M' | 'Q' | 'H'): QRCode {
  ...
  var setupPositionProbePattern = function (row: number, col: number) {
    for (let r = -1; r <= 7; r += 1) {
  ...
  _this.createTableTag = function (cellSize: number, margin?: number) {
    cellSize = cellSize || 2
...
var QRRSBlock = (function () {
  ...
  const getRsBlockTable = function (typeNumber: number, errorCorrectLevel: number) {
  ...
  _this.getRSBlocks = function (typeNumber: number, errorCorrectLevel: number) {
...
const byteArrayOutputStream = function () {
  ...
  _this.writeByte = function (b: number) {
    _bytes.push(b & 0xff)
  }

  _this.writeShort = function (i: number) {
    _this.writeByte(i)
    _this.writeByte(i >>> 8)
  }

  _this.writeBytes = function (b: number[], off: number, len: number) {
    off = off || 0
    len = len || b.length
  ...
  _this.writeString = function (s: string) {
    for (let i = 0; i < s.length; i += 1) {
...
const base64EncodeOutputStream = function () {
  ...
  _this.writeByte = function (n: number) {
...
var base64DecodeInputStream = function (str: string) {
  const _str = str
  let _pos = 0
  let _buffer = 0
  let _buflen = 0
...
const gifImage = function (width: number, height: number) {
  ...
  _this.setPixel = function (x: number, y: number, pixel: number) {
    _data[y * _width + x] = pixel
  }

  _this.write = function (out: any, blackColor = '#000000', whiteColor = '#ffffff') {
    // ---------------------------------
    // GIF Signature
  ...
  const bitOutputStream = function (out: any) {
    const _out = out
    let _bitLength = 0
    let _bitBuffer = 0
    ...
    _this.write = function (data: number, length: number) {
      if (data >>> length != 0) {
  ...
  var getLZWRaster = function (lzwMinCodeSize: number) {
  ...
  var lzwTable = function () {
    ...
    _this.indexOf = function (key: string) {
      return _map[key]
    }

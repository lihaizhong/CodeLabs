export class Config {
  ...
  public register(config: Partial<PlayerConfig>) {
    ...
    if (config.fillMode && [PLAYER_FILL_MODE.FORWARDS, PLAYER_FILL_MODE.BACKWARDS].includes(config.fillMode)) {
      this.fillMode = config.fillMode
    }

    if (config.playMode && [PLAYER_PLAY_MODE.FORWARDS, PLAYER_PLAY_MODE.FALLBACKS].includes(config.playMode)) {
  ...
  getConfig(entity: Video) {
    const {
      playMode, loopStartFrame, startFrame, endFrame, fillMode, loop,
    } = this
